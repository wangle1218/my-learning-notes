...
在 SVD 中，预测函数 (公式3-2) 中，在预测用户 i 对商品 j 的评分时，只考虑了用户特征向量和商品特征向量，以及评分相对于平均分的偏置向量。在SVD++ 中，更进一步的多考虑了用户对其所有有过评分行为的商品的隐式反馈。预测函数 p 变为以下形式：

$$p(U_i,M_j) = \mu + b_i + b_u + M_j^T (U_i + \frac{1}{\sqrt{|N_i|^2}}*\sum_{j\in N_i}y_j)$$

这里的 $$N_i$$ 表示用户 i 行为记录（包括浏览的和评过分的商品集合）。$$y_j$$ 为隐藏的评价了物品 j 的个人喜好偏置。这里除以$$\sqrt{|N_i|^2}$$ 的目的是为了抑制用户的评分商品数量对模型的影响。

$$y_j$$ 的更新公式为：

$$y_j := y_j + \eta \{[\sum_{j\in M,i\in U}(p(U_i,M_j)-V_{ij})*q_i \frac{1}{\sqrt{|N_i|^2}}]-\lambda *y_j\}$$

SVD++ 中的代价函数就是将 SVD 的代价函数替换一下预测函数部分为上式就行了。